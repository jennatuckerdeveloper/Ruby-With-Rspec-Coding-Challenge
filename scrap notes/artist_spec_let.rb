...
=begin
  This does not seem like correct usage.
  This does not want a brand new, fresh state every time.  
  It wants to accumulate actions on state across examples. 
  And I think that concept is a context.  
=end 

describe Artist do

  let(:name1) {'Sadie Smith'}
  let(:genre_folk) {'Folk'}
  let(:artist1 ) { Artist.new(name: name1) } 

  def add_two_songs_folk_genre(artist)
    genre_folk = 'Folk'
    folkSong1 = Song.new(name: 'Blue Song', genre: genre_folk)
    folkSong2 = Song.new(name: 'Gray Song', genre: genre_folk)
    artist.add_song(song: folkSong1)
    artist.add_song(song: folkSong2)
  end
